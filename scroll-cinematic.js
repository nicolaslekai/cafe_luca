...
function initScrub(cfg) {
  ...
  const images = [];
  let firstDrawn = cfg.frameCount === 0; // no frames yet -> overlay-only mode, poster image stays visible
  for (let i = 0; i < cfg.frameCount; i++) {
    const img = new Image();
    img.src = cfg.framePath(i + 1);
    img.onload = () => { if (!firstDrawn) { firstDrawn = true; draw(0); canvas.classList.add("ready"); } };
    images[i] = img;
  }
  let current = -1;
  function draw(index) {
  ...
  function update() {
    ...
    for (const el of lines) {
      ...
      // trapezoid: 20% ramps with a solid hold; lines pinned to 0/1 start/end visible
      const ramp = Math.max((b - a) * 0.2, 0.001);
      const rise = a === 0 ? 1 : (p - a) / ramp;
      const fall = b === 1 ? 1 : (b - p) / ramp;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fallback sweep: reveal anything in view that the observer missed
  // (scroll restoration after reload, deep links, mid-page loads).
  function sweepReveals() {
    document.querySelectorAll(".reveal:not(.in), .stat-num:not(.in)").forEach((el) => {
  ...
  window.addEventListener("load", () => setTimeout(sweepReveals, 100));
